...
class LibraryResolver(
  ...
  private val byName: Map[TsIdentLibrary, LibTsSource] =
    (Map.empty[TsIdentLibrary, LibTsSource] ++ allSources.groupBy(_.libName).map { case (k, v) => k -> v.head })
      .updated(TsIdent.std, stdLib)
...
object LibraryResolver {
  ...
  def moduleNameFor(source: LibTsSource, file: InFile): IArray[TsIdentModule] = {
    ...
    val longName: TsIdentModule = {
      ...
      ModuleNameParser(
        source.libName.`__value` +: file.path.relativeTo(source.folder.path).segments.toList,
        keepIndexPath,
      )
    }
